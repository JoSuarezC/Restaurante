...
public class ClientLocalPayment_Controller {
    ...
    void payOrder(ActionEvent event) {
        if (!getCheckBox().equals("Sin selección")) {
            ...
            orderID = ConnectionDB.getInstance().makeOrder(User.getCurrentUser().getUserID(), dateFormat.format(fecha), "Orden Local", totalAPagar, User.getCurrentUser().getSucursalName(), "Orden Local");
